...
class DepthOccupancyMap {
 public:
  ...
  void transformCloud(const laserCloud::Ptr &cloud_in,
                      laserCloud::Ptr &cloud_out, const int &flag) {
    if (nullptr == ptr_camera_calibrate_) {
      return;
    }
    laserCloud::Ptr ground_cloud(new laserCloud());
    laserCloud::Ptr laser_cloud(new laserCloud());
  ...
  // bool updateGlobalMap(
  //     const std::map<int, slam2d_core::common::TrajectoryNodePose>
  //         &node_poses) {
  //   return true;
  // }
  ...
  void saveGlobalMapYamlFile(const UserOccupancyGrid &map,
    ...
    // LOG(INFO) << "Writing map occupancy data yaml to " << mapmetadatafile;
  ...
  void saveGlobalMapFile(const UserOccupancyGrid &map,
    ...
    std::string mapdatafile = map_dir + ".pgm";
  ...
  UserOccupancyGrid getGlobalMap() {
 ...
 private:
  ...
  std::atomic<bool> mapping_mode_ = false;

  std::mutex insert_cloud_mtx_;
