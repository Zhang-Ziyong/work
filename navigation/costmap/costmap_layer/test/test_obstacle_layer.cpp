...
class obstacle_layer_tester : public testing::Test {
 public:
  virtual void TestBody() {}
  obstacle_layer_tester() {}

  void test_initialize() {
    auto ptr_obstacle_layer = std::make_shared<ObstacleLayer>();
    ...
    ptr_costmap_params->setParam("obstacle_layer.marking_cloud_sources",
                                 std::string("scan back_scan"));
    ptr_costmap_params->setParam("obstacle_layer.clearing_cloud_sources",
                                 std::string("scan front_scan"));

    ASSERT_EQ(ptr_obstacle_layer->initialize("obstacle_layer"), true);

    // two member
    ASSERT_EQ(ptr_obstacle_layer->v_marking_cloud_names_.size(), 2);
    ASSERT_EQ(ptr_obstacle_layer->v_clearing_cloud_names_.size(), 2);

    std::vector<std::shared_ptr<const CostmapCloud>> v_clouds,
        v_clearing_clouds;
    ptr_obstacle_layer->getMarkingClouds(v_clouds);
    ptr_obstacle_layer->getClearingClouds(v_clearing_clouds);
  ...
  void test_updateRaytraceBounds() {
    ...
    // greater than range
    ptr_obstacle_layer->updateRaytraceBounds(wp_origin, wp_point, range,
                                             cb_costmap_bound);
  ...
  void test_raytraceFreespace() {
    ...
    // outside the map
    clearing_clouds.origin_ = {-10.0, -10.0};
    ptr_obstacle_layer->raytraceFreespace(clearing_clouds, cb_costmap_bound);
    ...
    ptr_obstacle_layer->raytraceFreespace(clearing_clouds2, cb_costmap_bound2);
    // touch min
    ...
    ptr_obstacle_layer->raytraceFreespace(clearing_clouds2, cb_costmap_bound3);
    // limit the range
    bound = {-2.0, -1.0, 2.0, 3.0};
    ASSERT_EQ_BOUND(cb_costmap_bound3, bound);

    ptr_obstacle_layer->setDefaultValue(LETHAL_OBSTACLE);
    ptr_obstacle_layer->resizeMap(10, 10, 1, 0.0, 0.0);
    ...
    ptr_obstacle_layer->raytraceFreespace(clearing_clouds4, cb_costmap_bound4);

    bound = {0.0, 0.0, 4.0, 4.0};
    ASSERT_EQ_BOUND(cb_costmap_bound4, bound);

    printMap(*ptr_obstacle_layer);
    ASSERT_EQ(9, countValues(*ptr_obstacle_layer, FREE_SPACE));
  }
  ...
  void test_updateBounds() {
    ...
    ptr_obstacle_layer->setDefaultValue(NO_INFORMATION);
    ptr_obstacle_layer->resizeMap(10, 10, 1, 0.0, 0.0);
    ...
    ptr_obstacle_layer->updateBounds({5.0, 5.0}, cb_costmap_bound);
    printMap(*ptr_obstacle_layer);
    ASSERT_EQ(8, countValues(*ptr_obstacle_layer, FREE_SPACE));
    ASSERT_EQ(2, countValues(*ptr_obstacle_layer, LETHAL_OBSTACLE));
  ...
  void test_updateSpeedLevel() {
    ...
    ASSERT_EQ(ptr_obstacle_layer->initialize("obstacle_layer"), true);
    setArea(ptr_obstacle_layer->stop_bound_, -0.8, -1.0, 1.6, 1.0);
    // clear the origin stop area
    setArea(ptr_obstacle_layer->origin_stop_bound_, -0.8, -1.0, 1.6, 1.0);

    setArea(ptr_obstacle_layer->slow_bound_, -1.0, -1.3, 1.8, 1.3);
    // clear the origin slow down area
    setArea(ptr_obstacle_layer->origin_slow_bound_, -1.0, -1.3, 1.8, 1.3);

    auto func = [&ptr_obstacle_layer](
      ...
      ASSERT_EQ(ptr_obstacle_layer->updateSpeedLevel(wm_point, speedLevel),
                expect_result);
  ...
  void test_getSpeedLevel() {
    ...
    auto func = [&ptr_obstacle_layer](
      ...
      ptr_obstacle_layer->getSpeedLevel(ptr_field, speedLevel, slow_down, stop);
      ASSERT_EQ(speedLevel, expect_value);
    };
    ...
    // obstacle in current layer, the SpeedTwo will still remain the value
    ...
    ptr_obstacle_layer->getSpeedLevel(ptr_field, speedLevel,
                                      extra_slow_down_area, extra_stop_area);
...
auto ptr_obstacle_layer_tester = std::make_shared<obstacle_layer_tester>();

TEST(testCase, test_initialize) {
  ptr_obstacle_layer_tester->test_initialize();
}

TEST(testCase, test_updateRaytraceBounds) {
  ptr_obstacle_layer_tester->test_updateRaytraceBounds();
}

TEST(testCase, test_raytraceFreespace) {
  ptr_obstacle_layer_tester->test_raytraceFreespace();
}

TEST(testCase, test_updateBounds) {
  ptr_obstacle_layer_tester->test_updateBounds();
}

TEST(testCase, test_updateSpeedLevel) {
  ptr_obstacle_layer_tester->test_updateSpeedLevel();
}

TEST(testCase, test_getSpeedLevel) {
  ptr_obstacle_layer_tester->test_getSpeedLevel();
}
}  // namespace CVTE_BABOT
