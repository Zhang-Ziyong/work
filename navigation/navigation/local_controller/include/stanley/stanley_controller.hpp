...
//   std::string msg;
// };
enum CtrlMode { STANLEY_, LYPU_ };
class StanleyController : public LocalController {
 public:
  StanleyController();
  StanleyController(const StanleyController &obj) = delete;
  StanleyController &operator=(const StanleyController &obj) = delete;
  /**
   * computeMoveComand
   * @brief
 ...
 private:
  ...
  bool findTrackTarget(const Pose2d &c_pose, size_t &path_point_index,
                       size_t &last_point_index, double front_distance = 0.12,
                       double sum_dist = 1.0);
  /**
   * changeCtrl
   * @brief
  ...
  double psd_forward_install_;
  double psd_back_install_;
