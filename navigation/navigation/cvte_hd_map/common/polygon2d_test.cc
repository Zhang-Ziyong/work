...
TEST(Polygon2dTest, DistanceToPoint) {
  ...
  for (int iter = 0; iter < 10000; ++iter) {
    const double x = math_utils::RandomDouble(-10, 10);
    const double y = math_utils::RandomDouble(-10, 10);
    EXPECT_NEAR(poly1.DistanceTo({x, y}), box1.DistanceTo({x, y}), 1e-5);
  }
  for (int iter = 0; iter < 100; ++iter) {
    const double center_x = math_utils::RandomDouble(-10, 10);
    const double center_y = math_utils::RandomDouble(-10, 10);
    const double heading = math_utils::RandomDouble(0, M_PI * 2.0);
    const double length = math_utils::RandomDouble(1, 5);
    const double width = math_utils::RandomDouble(1, 5);
    const Box2d box({center_x, center_y}, heading, length, width);
    const Polygon2d polygon(box);
    for (int iter2 = 0; iter2 < 100; ++iter2) {
      const double x = math_utils::RandomDouble(-20, 20);
      const double y = math_utils::RandomDouble(-20, 20);
...
TEST(Polygon2dTest, ConvexHull) {
  ...
  for (int iter = 0; iter < 10000; ++iter) {
    const int kRange = 10;
    const int n = math_utils::RandomInt(3, 10);
    std::vector<Vec2d> points;
    for (int i = 0; i < n; ++i) {
      points.emplace_back(math_utils::RandomInt(0, kRange), math_utils::RandomInt(0, kRange));
    }
    double area = 0;
...
TEST(Polygon2dTest, Overlap) {
  ...
  for (int iter = 0; iter < 10000; ++iter) {
    const double x1 = math_utils::RandomDouble(-10, 10);
    const double y1 = math_utils::RandomDouble(-10, 10);
    const double x2 = math_utils::RandomDouble(-10, 10);
    const double y2 = math_utils::RandomDouble(-10, 10);
    const double heading1 = math_utils::RandomDouble(0, M_PI * 2.0);
    const double heading2 = math_utils::RandomDouble(0, M_PI * 2.0);
    const double l1 = math_utils::RandomDouble(1, 5);
    const double l2 = math_utils::RandomDouble(1, 5);
    const double w1 = math_utils::RandomDouble(1, 5);
    const double w2 = math_utils::RandomDouble(1, 5);
  ...
  for (int iter = 0; iter < 10000; ++iter) {
    const int kRange = 10;
    const int n1 = math_utils::RandomInt(3, 10);
    const int n2 = math_utils::RandomInt(3, 10);
    std::vector<Vec2d> points1, points2;
    for (int i = 0; i < n1; ++i) {
      points1.emplace_back(math_utils::RandomInt(0, kRange), math_utils::RandomInt(0, kRange));
    }
    for (int i = 0; i < n2; ++i) {
      points2.emplace_back(math_utils::RandomInt(0, kRange), math_utils::RandomInt(0, kRange));
    }
    Polygon2d polygon1;
    Polygon2d polygon2;
...
TEST(Polygon2dTest, BoundingBox) {
  ...
  for (int iter = 0; iter < 1000; ++iter) {
    const int num_sample_points = math_utils::RandomInt(3, 10);
    std::vector<Vec2d> points;
    for (int i = 0; i < num_sample_points; ++i) {
      const double x = math_utils::RandomInt(-10, 10);
      const double y = math_utils::RandomInt(-10, 10);
