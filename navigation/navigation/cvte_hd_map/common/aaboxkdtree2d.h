...
#ifndef AABOXKDTREE_HPP_
#define AABOXKDTREE_HPP_ 
...
class AABoxKDTree2dNode {
 public:
  ...
  std::vector<ObjectPtr> GetObjects(const Vec2d &point,
    ...
    GetObjectsInternal(point, distance, math_utils::Square(distance), &result_objects);
 ...
 private:
  ...
  void GetNearestObjectInternal(const Vec2d &point,
    ...
    if (LowerDistanceSquareToPoint(point) >= *min_distance_sqr - math_utils::kMathEpsilon) {
      return;
    }
    const double pvalue = (partition_ == PARTITION_X ? point.x() : point.y());
    ...
    if (*min_distance_sqr <= math_utils::kMathEpsilon) {
      return;
    }
    ...
    if (search_left_first) {
      for (int i = 0; i < num_objects_; ++i) {
        ...
        if (bound > pvalue && math_utils::Square(bound - pvalue) > *min_distance_sqr) {
          break;
        }
        ObjectPtr object = objects_sorted_by_min_[i];
