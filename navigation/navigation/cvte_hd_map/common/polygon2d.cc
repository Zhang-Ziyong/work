...
// const double kMathEpsilon = 1.0e-9;
...
double Polygon2d::DistanceTo(const LineSegment2d &line_segment) const {
  if (line_segment.length() <= math_utils::kMathEpsilon) {
...
bool Polygon2d::IsPointIn(const Vec2d &point) const {
  ...
  for (int i = 0; i < num_points_; ++i) {
    if ((points_[i].y() > point.y()) != (points_[j].y() > point.y())) {
      const double side = math_utils::CrossProd(point, points_[i], points_[j]);
...
bool Polygon2d::HasOverlap(const Polygon2d &polygon) const {
  ...
  return DistanceTo(polygon) <= math_utils::kMathEpsilon;
}
...
bool Polygon2d::Contains(const LineSegment2d &line_segment) const {
  ...
  if (!is_convex_) {
    ...
    return total_length >= line_segment.length() - math_utils::kMathEpsilon;
  }
  return true;
}
...
bool Polygon2d::Contains(const Polygon2d &polygon) const {
  // CHECK_GE(points_.size(), 3U);
  if (area_ < polygon.area() - math_utils::kMathEpsilon) {
    return false;
  }
  if (!IsPointIn(polygon.points()[0])) {
    return false;
...
void Polygon2d::BuildFromPoints() {
  ...
  for (int i = 1; i < num_points_; ++i) {
    area_ += math_utils::CrossProd(points_[0], points_[i - 1], points_[i]);
  }
  if (area_ < 0) {
    area_ = -area_;
  ...
  is_convex_ = true;
  for (int i = 0; i < num_points_; ++i) {
    if (math_utils::CrossProd(points_[Prev(i)], points_[i], points_[Next(i)]) <=
        -math_utils::kMathEpsilon) {
      is_convex_ = false;
      break;
    }
  }
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d> &points,
  ...
  std::sort(sorted_indices.begin(), sorted_indices.end(),
            [&](const int idx1, const int idx2) {
              ...
              if (std::abs(dx) > math_utils::kMathEpsilon) {
                return dx < 0.0;
              }
              return pt1.y() < pt2.y();
            });
  ...
  for (int i = 0; i < n + n; ++i) {
    ...
    while (count > last_count &&
           math_utils::CrossProd(points[results[count - 2]], points[results[count - 1]],
                     pt) <= math_utils::kMathEpsilon) {
      results.pop_back();
      --count;
    }
...
bool Polygon2d::ClipConvexHull(const LineSegment2d &line_segment,
  ...
  for (size_t i = 0; i < n; ++i) {
    prod[i] = math_utils::CrossProd(line_segment.start(), line_segment.end(), (*points)[i]);
    if (std::abs(prod[i]) <= math_utils::kMathEpsilon) {
      side[i] = 0;
    } else {
      side[i] = ((prod[i] < 0) ? -1 : 1);
    }
...
bool Polygon2d::GetOverlap(const LineSegment2d &line_segment,
  ...
  return min_proj <= max_proj + math_utils::kMathEpsilon;
}
...
std::vector<LineSegment2d> Polygon2d::GetAllOverlaps(
  ...
  for (size_t i = 0; i + 1 < projections.size(); ++i) {
    ...
    if (end_proj - start_proj <= math_utils::kMathEpsilon) {
      continue;
    }
    ...
    if (overlaps.empty() ||
        start_proj > overlaps.back().second + math_utils::kMathEpsilon) {
...
Box2d Polygon2d::BoundingBoxWithHeading(const double heading) const {
  // CHECK_GE(points_.size(), 3U);
  const Vec2d direction_vec(cos(heading), sin(heading));
  Vec2d px1;
  Vec2d px2;
  Vec2d py1;
  Vec2d py2;
  ...
  const double x1 = px1.transpose() * (direction_vec);
  const double x2 = px2.transpose() * (direction_vec);
  const double y1 = math_utils::CrossProd(py1.x(), py1.y(), direction_vec.x(), direction_vec.y());
  const double y2 = math_utils::CrossProd(py2.x(), py2.y(), direction_vec.x(), direction_vec.y());
...
Polygon2d Polygon2d::ExpandByDistance(const double distance) const {
  ...
  for (int i = 0; i < num_points_; ++i) {
    ...
    const double diff = math_utils::WrapAngle(end_angle - start_angle);
    if (diff <= math_utils::kMathEpsilon) {
      points.push_back(points_[i] +
                       Vec2d(cos(start_angle), sin(start_angle)) * distance);
    } else {
      const int count = static_cast<int>(diff / kMinAngle) + 1;
      for (int k = 0; k <= count; ++k) {
        const double angle = start_angle + diff * static_cast<double>(k) /
                                               static_cast<double>(count);
        points.push_back(points_[i] + Vec2d(cos(angle), sin(angle)) * distance);
      }
    }
  }
