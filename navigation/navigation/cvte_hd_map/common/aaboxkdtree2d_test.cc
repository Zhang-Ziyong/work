...
TEST(AABoxKDTree2dNode, OverallTests) {
  ...
  for (int num_boxes : kNumBoxes) {
    ...
    for (int i = 0; i < num_boxes; ++i) {
      const double cx = math_utils::RandomDouble(-kSize, kSize);
      const double cy = math_utils::RandomDouble(-kSize, kSize);
      const double dx = math_utils::RandomDouble(-kSize / 10.0, kSize / 10.0);
      const double dy = math_utils::RandomDouble(-kSize / 10.0, kSize / 10.0);
    ...
    for (int i = 0; i < kNumQueries; ++i) {
      const Vec2d point(math_utils::RandomDouble(-kSize * 1.5, kSize * 1.5),
                        math_utils::RandomDouble(-kSize * 1.5, kSize * 1.5));
    ...
    for (int i = 0; i < kNumQueries; ++i) {
      const Vec2d point(math_utils::RandomDouble(-kSize * 1.5, kSize * 1.5),
                        math_utils::RandomDouble(-kSize * 1.5, kSize * 1.5));
      const double distance = math_utils::RandomDouble(0, kSize * 2.0);
      for (int k = 0; k < kNumTrees; ++k) {
