...
// const double kMathEpsilon = 1e-9;
...
AABox2d::AABox2d(const Vec2d &center, const double length, const double width)
  ...
  CHECK_GT(length_, -math_utils::kMathEpsilon);
  CHECK_GT(width_, -math_utils::kMathEpsilon);
}
...
AABox2d::AABox2d(const std::vector<Vec2d> &points) {
  if(points.empty()) {
    LOG(ERROR) << "empty points can not construct AABox2d, will crash ";
  }
...
bool AABox2d::IsPointIn(const Vec2d &point) const {
  return std::abs(point.x() - center_.x()) <= half_length_ + math_utils::kMathEpsilon &&
         std::abs(point.y() - center_.y()) <= half_width_ + math_utils::kMathEpsilon;
}
