...
bool Box2d::IsPointIn(const Vec2d &point) const {
  ...
  return dx <= half_length_ + math_utils::kMathEpsilon && dy <= half_width_ + math_utils::kMathEpsilon;
}
...
bool Box2d::IsPointOnBoundary(const Vec2d &point) const {
  ...
  const double dx = std::abs(x0 * cos_heading_ + y0 * sin_heading_);
  const double dy = std::abs(x0 * sin_heading_ - y0 * cos_heading_);
  return (std::abs(dx - half_length_) <= math_utils::kMathEpsilon &&
          dy <= half_width_ + math_utils::kMathEpsilon) ||
         (std::abs(dy - half_width_) <= math_utils::kMathEpsilon &&
          dx <= half_length_ + math_utils::kMathEpsilon);
}
...
bool Box2d::HasOverlap(const LineSegment2d &line_segment) const {
  ...
  return DistanceTo(line_segment) <= math_utils::kMathEpsilon;
}
      ...
      case 2:
        return (x1 > x2) ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                         ...
                         : PtSegDistance(box_x, box_y, x1, y1, x2, y2,
                                         line_segment.length());
      case -1:
        return math_utils::CrossProd({x1, y1}, {x2, y2}, {box_x, -box_y}) >= 0.0
                   ? 0.0
                   : PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                                   line_segment.length());
      case -4:
        return math_utils::CrossProd({x1, y1}, {x2, y2}, {box_x, -box_y}) <= 0.0
                   ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                                   line_segment.length())
                   : (math_utils::CrossProd({x1, y1}, {x2, y2}, {-box_x, box_y}) <= 0.0
                          ? 0.0
                          : PtSegDistance(-box_x, box_y, x1, y1, x2, y2,
                                          line_segment.length()));
    }
...
void Box2d::RotateFromCenter(const double rotate_angle) {
  heading_ = math_utils::NormalizeAngle(heading_ + rotate_angle);
