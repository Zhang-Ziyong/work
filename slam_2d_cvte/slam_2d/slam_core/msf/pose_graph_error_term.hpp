...
class CornerPointFactor : public ceres::SizedCostFunction<3, 12> {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  CornerPointFactor(const Vec3d &curr_point, const Vec3d &map_point,
                    const double scale)
  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals,
    ...
    if (jacobians != nullptr) {
      if (jacobians[0]) {
        ...
        J0.setZero();
        J0.block<3, 3>(0, 0) =
            -scale * Mathbox::skew(point_w - pose.block<3, 1>(0, 3));
...
struct PointDistanceFactor {
  PointDistanceFactor(Eigen::Vector3d curr_point_,
  ...
  bool operator()(const T *q, const T *t, T *residual) const {
    ...
    residual[0] = (point_w - closed_point).norm();
...
struct PointToLineDistanceFactor {
  PointToLineDistanceFactor(Eigen::Vector3d curr_point_,
  ...
  bool operator()(const T *q, const T *t, T *residual) const {
    ...
    Eigen::Matrix<T, 3, 1> cp{T(curr_point.x()), T(curr_point.y()),
                              T(curr_point.z())};
    Eigen::Matrix<T, 3, 1> lpa{T(closed_point_a.x()), T(closed_point_a.y()),
                               T(closed_point_a.z())};
    Eigen::Matrix<T, 3, 1> lpb{T(closed_point_b.x()), T(closed_point_b.y()),
                               T(closed_point_b.z())};
    Eigen::Matrix<T, 3, 1> point_w;
    point_w = q_w_curr * cp + t_w_curr;

    Eigen::Matrix<T, 3, 1> nu =
        (point_w - lpa).cross(point_w - lpb);  // 向量OA 叉乘 向量OB
...
struct RelativeRFactor {
  ...
  static ceres::CostFunction *Create(const double q_x, const double q_y,
...
struct RelativeTFactor {
  ...
  static ceres::CostFunction *Create(const double t_x, const double t_y,
                                     const double t_z, const double t_var) {
...
struct RelativeRTFactor {
  RelativeRTFactor(double t_x, double t_y, double t_z, double q_x, double q_y,
                   double q_z, double q_w, double t_var, double q_var)
      : t_x(t_x),
        t_y(t_y),
        t_z(t_z),
        q_x(q_x),
  ...
  static ceres::CostFunction *Create(const double t_x, const double t_y,
                                     const double t_z, const double q_x,
                                     const double q_y, const double q_z,
                                     const double q_w, const double t_var,
                                     const double q_var) {
