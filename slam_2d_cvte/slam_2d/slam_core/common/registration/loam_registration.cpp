...
bool LoamRegistration::loam_registration(const laserCloud::Ptr cur_corner_cloud, const laserCloud::Ptr cur_surf_cloud, const laserCloud::Ptr map_corner_cloud, const laserCloud::Ptr map_surf_cloud, Mat34d &cur_pose)
{
...
void LoamRegistration::findCorrespondingFeature()
{
  ...
  for (int i = 0; i < cur_corner_cloud_num; i++)
  {
    ...
    if (point_search_sqdis[4] < 1.0)
    {
      ...
      for (int j = 0; j < 5; j++)
      {
        Vec3d temp_point(ds_map_corner_cloud_->points[point_search_ind[j]].x, ds_map_corner_cloud_->points[point_search_ind[j]].y, ds_map_corner_cloud_->points[point_search_ind[j]].z);
        center += temp_point;
  ...
  for (int i = 0; i < cur_surf_cloud_num; i++)
  {
    ...
    if (point_search_sqdis[4] < 1.0)
    {
      ...
      for (int j = 0; j < 5; j++)
      {
        if (fabs(norm(0) * ds_map_surf_cloud_->points[point_search_ind[j]].x + norm(1) * ds_map_surf_cloud_->points[point_search_ind[j]].y + norm(2) * ds_map_surf_cloud_->points[point_search_ind[j]].z + negative_OA_dot_norm) > 0.2)
        {
          plane_valid = false;
          break;
        }
      ...
      if (plane_valid)
      {
        ...
        if (scale > 0.1)
        {
          ceres::CostFunction *cost_function = new PlaneFactor(curr_point, norm, scale, negative_OA_dot_norm);
          problem.AddResidualBlock(cost_function, loss_function, keyframe_pose_.data());
        }
      }
    }
