...
class Transformbox {
 public:
  ...
  static void pointXYZAssociateToMap(depthPointType const *const pi,
                                     depthPointType *const po,
                                     const Mat34d &pose) {
    Vec3d point_curr(pi->x, pi->y, pi->z);
    Vec3d point_w = Mathbox::multiplePoint(pose, point_curr);
    po->x = point_w.x();
  ...
  static depthCloud::Ptr transformDepthCloud(const Mat34d &pose,
                                             depthCloud::Ptr cloudIn) {
