...
class Mathbox
{
 public:
  ...
  static Mat34d multiplePose34d(const Mat34d &pose1, const Mat34d &pose2)
  {
    ...
    ret.block<3, 1>(0, 3) = pose1.block<3, 3>(0, 0) * pose2.block<3, 1>(0, 3) + pose1.block<3, 1>(0, 3);
    return ret;
  }
  ...
  {
    Mat34d ret = Identity34();
    Mat3d R = (Eigen::AngleAxisd(angles[0], Vec3d::UnitZ()) * Eigen::AngleAxisd(angles[1], Vec3d::UnitY()) * Eigen::AngleAxisd(angles[2], Vec3d::UnitX())).toRotationMatrix();
    ret.block<3, 3>(0, 0) = R;
    ret.block<3, 1>(0, 3) = t;
    return ret;
  }
   ...
   *@return Mat34d
   **/
  static Mat34d deltaPose34d(const Mat34d &pose1, const Mat34d &pose2) { return multiplePose34d(inversePose34d(pose1), pose2); }
  ...
  static Mat3d Jr_inv(const Vec3d &w)
  {
    ...
    else
    {
      ...
      return Mat3d::Identity() + 0.5 * skew(w) + (1.0 - (1.0 + cos(theta)) * theta / (2.0 * sin(theta))) * K * K;
    }
  }
