...
struct OdomMeasure
{
  ...
  OdomMeasure(const Mat34d &pose, const Eigen::Quaterniond Q_gyr_only, const double time_stamp) : pose(pose), Q_gyr_only(Q_gyr_only), time_stamp(time_stamp) {}
  OdomMeasure(const Mat34d &pose, const double linear, const double angular, const double time_stamp) : pose(pose), linear(linear), angular(angular), time_stamp(time_stamp) {}
  Mat34d pose;
...
struct ImuMeasure
{
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ImuMeasure(){};
  ImuMeasure(const Vec3d acc, const Vec3d gyr, const size_t count, const double time_stamp) : acc(acc), gyr(gyr), count(count), time_stamp(time_stamp) {}
 ...
 public:
  ...
  Mid_Filter(int size)
  {
    ...
    if (filter_size > buf_size)
    {
      ...
      printf("--Mid_Filter: filter_size exceed buf size, and filter_size is set 499 !!! \n");
    }
  }
...
{
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  GPSMeasure(const Vec3d &nav_pos, const double time_stamp, const double cov, unsigned int loc_type) : nav_pos(nav_pos), time_stamp(time_stamp), cov(cov), loc_type(loc_type) {}
  Vec3d nav_pos;
  double time_stamp;
  double cov;
...
{
  GpsMsg() = default;
  GpsMsg(const double latitude, const double longitude, const double altitude, const double cov, const double time_stamp, const unsigned int loc_type) : latitude(latitude), longitude(longitude), altitude(altitude), cov(cov), time_stamp(time_stamp), loc_type(loc_type) {}
  double latitude;        ///< 纬度
  double longitude;       ///< 经度
  double altitude;        ///< 高度
  double cov;             ///< 定位误差
...
struct CloudMeasure
{
  CloudMeasure(const laserCloud::Ptr cloud_in, const double time_stamp) : cloud(cloud_in), time_stamp(time_stamp) {}
...
struct RobotState
{
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  RobotState() = default;
  RobotState(const Mat34d &pose, const Vec3d &speed, const double time_stamp, const double cov) : pose(pose), speed(speed), time_stamp(time_stamp), cov(cov) {}
  void Reset()
  {
...
struct KeyFrameInfo
{
  ...
  KeyFrameInfo(const size_t frame_id, const Mat34d &pose, const double pose_cov, const bool has_gps = false, const Vec3d &gps_pos = Vec3d::Zero(), const double gps_cov = -1) : frame_id(frame_id), pose(pose), pose_cov(pose_cov), has_gps(has_gps), gps_pos(gps_pos), gps_cov(gps_cov) {}
  size_t frame_id;
  Mat34d pose;
  double pose_cov;
  bool has_gps;
