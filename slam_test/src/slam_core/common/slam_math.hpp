...
struct QuatPose {
  ...
  QuatPose(const Mat34d &pose)
      : q_wb(pose.block<3, 3>(0, 0)), t_wb(pose.block<3, 1>(0, 3)) {}
 ...
 public:
  ...
  static Mat3d Exp3d(const Vec3d &dx) {
    ...
    if (theta < SMALL_EPS) {
      ...
      return Mat3d::Identity() + sin(theta) * hatdx +
             (1 - cos(theta)) * hatdx * hatdx;
    }
  }
   ...
   **/
  static Mat3d Interp_SO3(const Mat3d &R1, const Mat3d &R2,
                          const double alpha) {
    if (alpha < 0.000001)
      return R1;
   ...
   *@param[out] alpha-插值系数
   **/
  static Mat34d Interp_SE3(const Mat34d &T1, const Mat34d &T2,
                           const double alpha) {
    ...
    pose.block<3, 1>(0, 3) =
        (1.0 - alpha) * T1.block<3, 1>(0, 3) + alpha * T2.block<3, 1>(0, 3);
    return pose;
  }
