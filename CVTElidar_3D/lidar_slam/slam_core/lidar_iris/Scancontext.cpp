...
bool SCManager::saveScancontext(const std::string &map_dir,
                                const std::vector<long int> &frame_ids) {
  LOG(INFO) << "SCManager::saveScancontext ";
  clock_t startTime = clock();
  if (polarcontexts_.empty()) {
    LOG(ERROR) << "polarcontexts_.empty()";
    return false;
  }
  std::ofstream feature_info_file;
  const std::string &info_file_name = map_dir + "sc_feature_info.txt";
  ...
  for (size_t i = 0; i < polarcontexts_.size(); i++) {
    long int frame_id = frame_ids[i];
    std::string feature_img_file =
        map_dir + "sc_featur_img" + std::to_string(frame_id) + ".xml";

    cv::Mat temp_mat;
    Eigen::MatrixXd temp_sc = polarcontexts_[i];
    temp_mat.create(PC_NUM_RING, PC_NUM_SECTOR, CV_32FC1);
    for (int i = 0; i < PC_NUM_RING; ++i) {
      for (int j = 0; j < PC_NUM_SECTOR; ++j) {
        temp_mat.at<float>(i, j) = temp_sc(i, j);
      }
    }
    cv::FileStorage fs(feature_img_file, cv::FileStorage::WRITE);
    fs << "scan_context" << temp_mat;
    fs.release();
    feature_info_file << std::setprecision(0) << frame_id << std::endl;
  }
  feature_info_file.close();
  clock_t endTime = clock();
  LOG(INFO) << "saveScancontext " << polarcontexts_.size() << ","
            << (endTime - startTime) / (double)CLOCKS_PER_SEC << "s.";
  return true;
}

bool SCManager::loadFeature(const std::string &map_dir,
                            std::vector<long int> &frame_ids) {
  std::string feature_file = map_dir;
  if (feature_file.back() != '/') {
    feature_file.push_back('/');
  }
  std::ifstream feature_info_file;
  const std::string &info_data_dir = map_dir + "sc_feature_info.txt";
  ...
  while (!feature_info_file.eof()) {
    ...
    cv::FileStorage fs;
    try {
      fs.open(feature_img_file, cv::FileStorage::READ);
    } catch (cv::Exception &e) {
      LOG(ERROR) << "cv::Exception caught " << e.what();
    ...
    cv::Mat temp_mat;
    fs["scan_context"] >> temp_mat;
    Eigen::MatrixXd sc_matrix(PC_NUM_RING, PC_NUM_SECTOR);
    for (int i = 0; i < PC_NUM_RING; ++i) {
      for (int j = 0; j < PC_NUM_SECTOR; ++j) {
        sc_matrix(i, j) = temp_mat.at<float>(i, j);
      }
    }

    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext(sc_matrix);
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext(sc_matrix);
    std::vector<float> polarcontext_invkey_vec = eig2stdvec(ringkey);

    polarcontexts_.push_back(sc_matrix);
    polarcontext_invkeys_.push_back(ringkey);
    polarcontext_vkeys_.push_back(sectorkey);
    polarcontext_invkeys_mat_.push_back(polarcontext_invkey_vec);
  }
  feature_info_file.close();
  clock_t endTime = clock();
  LOG(INFO) << "load feature data " << map_dir << ", " << polarcontexts_.size()
            << "," << (endTime - startTime) / (double)CLOCKS_PER_SEC << "s.";
  if (polarcontexts_.empty()) {
    return false;
  }
...
std::pair<int, float> SCManager::detectLoopClosureID(void) {
  ...
  if (min_dist < SC_DIST_THRES) {
    ...
    LOG(INFO) << "[Loop found] Nearest distance: " << min_dist << " btn "
              << polarcontexts_.size() - 1 << " and " << nn_idx << ".";
    LOG(INFO) << "[Loop found] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE
              << " deg.";
  } else {
    std::cout.precision(3);
    LOG(INFO) << "[Not loop] Nearest distance: " << min_dist << " btn "
              << polarcontexts_.size() - 1 << " and " << nn_idx << ".";
    LOG(INFO) << "[Not loop] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE
              << " deg.";
  }
...
std::pair<int, float> SCManager::findNearestID(
    pcl::PointCloud<SCPointType> &_scan_down,
    const std::vector<size_t> &search_frame_ids) {
  int loop_id{-1};  // init with -1, -1 means no loop (== LeGO-LOAM's variable
                    // "closestHistoryFrameID")

  Eigen::MatrixXd curr_desc =
      makeScancontext(_scan_down);  // current observation (query)
  ...
  std::vector<float> curr_key =
      eig2stdvec(ringkey);  // current observation (query)
   ...
   */
  TicToc t_calc_dist;
  for (int candidate_iter_idx = 0; candidate_iter_idx < search_frame_ids.size();
       candidate_iter_idx++) {
    MatrixXd polarcontext_candidate =
        polarcontexts_[search_frame_ids[candidate_iter_idx]];
    std::pair<double, int> sc_dist_result =
        distanceBtnScanContext(curr_desc, polarcontext_candidate);
    ...
    if (candidate_dist < min_dist) {
      ...
      nn_idx = search_frame_ids[candidate_iter_idx];
    }
    if (min_dist < SC_DIST_THRES) {
      break;
    }
   ...
   * loop threshold check
   */
  if (min_dist < FIND_NEAREST_THRES) {
    loop_id = nn_idx;
