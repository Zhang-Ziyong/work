...
class SCManager {
 ...
 public:
  ...
  const double LIDAR_HEIGHT =
      0.5;  // lidar height : add this for simply directly using lidar scan in
  ...
  const int NUM_EXCLUDE_RECENT =
      10;  // simply just keyframe gap, but node position distance-based
           // exclusion is ok.
  ...
  const double SC_DIST_THRES =
      0.2;  // 0.4-0.6 is good choice for using with robust kernel (e.g.,
            // Cauchy, DCS) + icp fitness threshold / if not, recommend 0.1-0.15
  const double FIND_NEAREST_THRES = 0.5;
  // config
  const int TREE_MAKING_PERIOD_ =
      10;  // i.e., remaking tree frequency, to avoid non-mandatory every
           ...
           // means the tree is updated evrey 10 sec. But you can use the
           // smaller value because it is enough fast ~ 5-50ms wrt N.
